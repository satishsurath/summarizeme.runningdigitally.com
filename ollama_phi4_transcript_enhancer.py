...
def chunk_text_by_sentence(text: str, max_tokens: int = 4000, model: str = "gpt-3.5-turbo") -> list:
    ...
    """
    # We split by period (.) to get sentences. 
    # You could also use more advanced sentence tokenizers (e.g. nltk) for better splitting.
    ...
    for sentence in sentences:
        # Add the period back to the end of the sentence (if it’s not empty).
        sentence = sentence.strip()
        if not sentence:
            continue

        # We'll count tokens in "sentence + '.'"
        sentence_with_period = sentence + "."
        sentence_tokens = num_tokens_from_string(sentence_with_period, model=model)

        # If adding this sentence would exceed our threshold, we close out the current chunk
        if current_length + sentence_tokens > max_tokens:
            # Add the current chunk to list
    ...
    # Add any leftover sentences in the last chunk
    if current_chunk:
        chunk_text = ". ".join(current_chunk).strip()
        if chunk_text:
            chunks.append(chunk_text)
...
Transcript:
{chunk_text}
REMEMBER TO FOLLOW THE RULES EXACTLY
    """
    ...
    # The Ollama API might return the content in different JSON structures. 
    # Adjust this retrieval as per your actual response shape:
...
def transcript_enhancer_ollama(
    ...
    if total_tokens > MAX_TOKENS_PER_CHUNK:
        ...
        for i, chunk in enumerate(text_chunks):
            print(f"Enhancing chunk {i+1}/{len(text_chunks)}...")
            enhanced_chunk = enhance_chunk_with_ollama(chunk, model=model)
        ...
        summary_text = enhance_chunk_with_ollama(transcript_text, model=model)
