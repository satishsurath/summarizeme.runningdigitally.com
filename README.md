...
Provide a channel URL (or a single video URL from that channel), and the application downloads transcripts for each video.
	•	No channel? No problem—just supply any video from a channel, and we will infer the channel ID.
	2.	Summarization (v2)
	•	Breaks large transcripts into 4,000-word chunks (approx) for safe LLM processing.
...
├─ app.py               # Main Flask app, with new routes and chat endpoints
...
│  ├─ status.html
│  ├─ videos.html
│  ├─ channel_chat.html   # New chat interface for entire channel
│  ├─ video_chat.html     # New chat interface for a single video
│  ├─ summary_v2.html     # View SummariesV2 details in markdown
...
### Database Schema (Simplified)
	...
	•	These store embeddings for chunked text, used for the chat retrieval.
...
Ensure your Postgres DB exists, then either:
	•	Use migrations if you have them set up, or
	•	Create tables from your models (e.g., in a Python shell, or from a script that calls Base.metadata.create_all(engine)).
	6.	Run the app:
Mac / Linux:
...
1. Downloading Transcripts
	...
	•	Enter a YouTube channel URL or video URL from that channel and click “Start Download.”
	•	Check the Status page (/status) to see progress.
	•	Once done, you can list videos under the videos page (/videos/<channel-name>).

2. Summarizing Videos (SummariesV2)
	•	From the “Videos” page for a channel, select videos to summarize (or set up a button that calls /api/summarize_v2).
	•	The system chunkifies transcripts and produces four types of summaries:
	1.	Concise Summary
	2.	Key Topics
	3.	Important Takeaways
	4.	Comprehensive Notes
	•	Progress updates are available at /api/summarize_v2/status/<task_id>.

3. Chatting With a Channel (/chat-channel/<channel_name>)
	•	Navigate to http://127.0.0.1:5000/chat-channel/<channel_name>.
	•	Type your query in the prompt. The system:
...
4. Chatting With a Single Video (/chat-video/<video_id>)
	•	Navigate to http://127.0.0.1:5000/chat-video/<video_id>.
	•	Enter your query, and the system will embed that query and retrieve the top chunks (from this one video’s transcript or chosen summary field).
...
REMOTE_OLLAMA_HOST	Hostname/IP for your remote Ollama service (port 11434).	localhost or remote.server.com

Troubleshooting / Tips
	1.	Ensure Ollama is Running
	•	By default, Ollama listens on port 11434. If you’re running Ollama remotely, confirm your firewall allows incoming requests or use SSH tunneling.
	2.	Check DB Connectivity
	•	Make sure DATABASE_URL is correct and the tables exist.
	•	If you run into table-not-found errors, ensure you created the tables with your models or migrations.
