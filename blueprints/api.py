            ...
            for vid in video_ids:
                ...
                if not existing_folder:
                    folder_assoc = VideoFolder(
                        folder_name=channel_name, video_id=vid, last_modified=datetime.datetime.now(datetime.timezone.utc)
                    )
...
def api_refresh_channel():
    ...
    try:
        ...
        task_id = f"refresh_{human_playlist_name}_{int(datetime.datetime.now(datetime.timezone.utc).timestamp())}"
        download_statuses[task_id] = {"status": "in_progress", "processed": 0, "total": 0, "errors": []}
